...
{
    public class BlobAnalyzeMarshaler_Base : ICustomMarshaler
    //public class BlobAnalyzeMarshaler: ArrayMarshaler<Blob_Info>
    {
        public BlobAnalyzeMarshaler_Base()
        {
        ...
        {
            return new BlobAnalyzeMarshaler_Base();
        }
        ...
        public object  MarshalNativeToManaged(IntPtr native_ptr)
        {
            try
            {
                ...
                var size = Marshal.SizeOf(Marshal.ReadIntPtr(native_ptr));

                IntPtr cur_native_ptr = native_ptr;// + sizeof(Int32);

                int blob_num = 400;
                Blob_Info_Base[] new_blob = new Blob_Info_Base[blob_num];
                unsafe
                {
                    for (int blob_id = 0; blob_id < blob_num; blob_id++)
                    {
                        Blob_Info_Base test_blob = new Blob_Info_Base();
                        void* native_blob = cur_native_ptr.ToPointer();
                        Marshal.PtrToStructure(cur_native_ptr, test_blob);
                        test_blob = (Blob_Info_Base)Marshal.PtrToStructure(cur_native_ptr, typeof(Blob_Info_Base));
                        new_blob[blob_id] = test_blob;

                        cur_native_ptr += Marshal.SizeOf<Blob_Info_Base>();
        ...
        public IntPtr MarshalManagedToNative(object managed_obj)
        {
            try
            {
                Blob_Info_Base[] blob_Infos = (Blob_Info_Base[]) managed_obj;
                ...
                {
                    //Marshal.Copy(blob_Infos[i]._blob_info, 0, yourABSObject.Data, length);
                    ...
                    Marshal.StructureToPtr(blob_Infos[i], _blob_info, false);
    ...
    ///////////////////////////////////////////////////////////////////////////
    ///
    public class BlobAnalyzeMarshaler_Contour : ICustomMarshaler
    //public class BlobAnalyzeMarshaler: ArrayMarshaler<Blob_Info>
    {
        public BlobAnalyzeMarshaler_Contour()
        {

        }

        //BlobAnalyzeMarshaler_Contour
        public static ICustomMarshaler GetInstance(string pstrCookie)
        {
            return new BlobAnalyzeMarshaler_Contour();
        }
        ...
        public object MarshalNativeToManaged(IntPtr native_ptr)
        {
            try
            {
                ...
                unsafe
                {
                    //Blob_Infos* test = (Blob_Infos*)native_ptr.ToPointer();
                    for (int blob_id = 0; blob_id < blob_num; blob_id++)
                    {
