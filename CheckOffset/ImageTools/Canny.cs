...
namespace OpenSource
{
    ...
    public class Canny 
    {
        ...
        public Canny(Bitmap Input)
        {
            ...
            MaxHysteresisThresh = 600F;
            MinHysteresisThresh = 400F;
            Obj = Input;
            Width = Obj.Width;
            Height = Obj.Height;
        ...
        private void ReadImage()
        {
            ...
            unsafe
            {
                ...
                int pixel_size = (bitmapData1.Stride / bitmapData1.Width);
                for (i = 0; i < bitmapData1.Height; i++)
                {
                    for (j = 0; j < bitmapData1.Width; j++)
                    {
                        ...
                        //4 bytes per pixel
                        imagePointer1 += pixel_size;
                    }//end for j
                    //4 bytes per pixel
                    imagePointer1 += bitmapData1.Stride - (bitmapData1.Width * pixel_size);
                }//end for i
            }//end unsafe
            image.UnlockBits(bitmapData1);
            return;
        ...
        private void DetectCannyEdges()
        {
           ...
           // NonMax = Gradient;
            ...
            MaxHysteresisThresh = 0;
            int greatest_Fix = 200;
            int grestest_num = 0;
            bool source_image_binary = false;
            for(int my = histogram.Histogram_Result.Length - 1; my >= 0; my--)
            {
                if (histogram.Histogram_Result[my] <= 0)
                    continue;

                if (MaxHysteresisThresh <= 0)
                    MaxHysteresisThresh = my;

                grestest_num++;

                if (!source_image_binary)
                {
                    if (greatest_Fix <= grestest_num)
                    {
                        if (my > MinHysteresisThresh)
                        {
                            MinHysteresisThresh = my;
                        }
                        break;
                    }
                }
            ...
            //MaxHysteresisThresh = histogram.Max_Histogram_Val;
            //MinHysteresisThresh = histogram.Histogram_Result[histogram.Max_Histogram_Val - 5]; // ²Ä 10 ­Ó
